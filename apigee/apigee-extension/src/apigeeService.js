...
module.exports =  class ApigeeService {
	...
	async _buildAssetParams(token, acccessToken) {	
		...
		for (let api of apis) {
			...
			generatedResources.push(apiDetails);
			...
			apiDetails.resources.push(apiService);
			...
			for (var entry of deployedRevisions) {
				...
				for (var revision of revisions) {
					...
					apiDetails.resources.push(apiServiceRevision);
					...
					for (var envHost of envHosts) {
						...
						apiDetails.resources.push(apiServiceInstance);
						...
						apiDetails.resources.push(consumerInstanceInstance);
					}
				}
			}
		}
