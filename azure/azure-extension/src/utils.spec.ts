...
describe('utils', () => {
	...
	it('exports configFilePath', () => {
		let expectedPath = path.join(os.homedir(), '.axway', 'azure-extension.json');
	...
	it('createSupportResources: creates env, subdef, webhook', async () => {
		...
		expect(commitToFs.lastCall.args[2][1].kind).to.equal('Webhook');
		expect(commitToFs.lastCall.args[2][2].kind).to.equal('ConsumerSubscriptionDefinition');
	})
